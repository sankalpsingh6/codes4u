...
<div class="super_container">
	...
	<div class="container" style="margin-top:150px; height:">
		<div class="row">
			<?php 
			$ext = $_SESSION['FROM'] != 'py'?$_SESSION['FROM']:'python';
			$beg = array();
			$exp = array();
			$query = "SELECT * FROM programs";
			$result = mysqli_query($cid,$query);
			while($row = mysqli_fetch_array($result)){
			if($row['number_of_star'] == '1'){
				$beg[$row['file']] = $row['title'];
			}else{
				$exp[$row['file']] = $row['title'];
			}}?>
			<div class="col-sm-6" style="min-height: 500px;height:auto;">
				<h3 style="margin-left:15px;"><ins>For Beginers</ins></h3>
				<?php foreach($beg as $key=>$val){?>
				<button type="button" class="btn btn-link question" name="<?php echo $key; ?>"><?php echo str_replace('*','In '.ucwords($ext),$val); ?></button>
				<br>
				<?php } ?>
			</div>
			<div class="col-sm-6" style="min-height: 500px;height:auto;">
				<h3 style="margin-left:15px;"><ins>For Experienced</ins></h3>
...
function logout(){
	...
	if (confirmlogout == true){
		$.ajax({
			...
			datatype:'text',
			success:function(){
				window.location = "specific-program-list.php";
			}
		});
	}
...
$("button.question").click(function(){
	$.ajax({
		url:"set-session.php",
		method:"post",
		data:"varname=FILENAME&vardata="+$(this).attr('name'),
		datatype:"text",
		success:function(){
